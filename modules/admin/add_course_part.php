...
if(isset($_POST['add_course_part'])){
    $errors = [];
    ...
    if(count($errors) == 0){
        ...
        $link->query ("INSERT INTO course_part (`part_title`,`course_id`, `headline_id` , `video` ,`time_video`,`file` ,`price_status`) VALUES ('".$_POST['title']."',".$_GET['id'].",".$_GET['headline_id'].",'".$new_video_name."',".$_POST['time'].",'".$new_file_name."',".$_POST['options'].")") ;
        if($link->errno == 0){
            $message = '<script>toastr.success("جلسه با موفقیت اضافه شد");</script>';
            $_POST['title'] = '';
        } else {
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-6 mt-3">
                                <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                                    <!-- رایگان button -->
                                    <input type="radio" class="btn-check" name="options" id="option1" value="0" checked>
                                    <label class="btn btn-sm btn-light btn-success-soft-check border-0 m-0" for="option1">رایگان</label>
                                    <!-- Premium button -->
                                    <input type="radio" class="btn-check" name="options" value="1" id="option2">
                            ...
                            <div class="d-flex justify-content-end mt-3">
                                <input type="submit" name="add_course_part" value="افزودن" class="btn btn-primary">&nbsp;
