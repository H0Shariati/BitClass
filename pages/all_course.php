...
if(isset($_POST['search'])){
    $result_course = $link->query("SELECT courses.* , course_category.category_name as category_name  FROM courses join course_category ON courses.cat_id=course_category.category_id where courses.course_title like '%".$_POST['search']."%' order by courses.update_date DESC");
    if($result_course->num_rows==0){?>
        <div class=" container alert alert-danger">نتیجه ای یافت نشد</div>
    <?php

    }
    $count_course = $link->query("SELECT count(*) as count FROM courses where courses.course_title like '%".$_POST['search']."%'");$row_count_course = $count_course->fetch_assoc();
    $number_of_course = $row_count_course['count'];
    $pages_number = ceil($number_of_course / 8);

}
else {
    $result_course = $link->query("SELECT courses.* , course_category.category_name as category_name  FROM courses join course_category ON courses.cat_id=course_category.category_id order by courses.update_date DESC");
    $count_course = $link->query("SELECT count(*) as count FROM courses");
    $row_count_course = $count_course->fetch_assoc();
    $number_of_course = $row_count_course['count'];
    $pages_number = ceil($number_of_course / 8);
}
        ...
        <div class="row">
            <div class="col-12">
                <div class="bg-light p-4 text-center rounded-3">
                    <h1 class="m-0 fs-2">همه دوره ها</h1>
                    <!-- Breadcrumb -->
                    <div class="d-flex justify-content-center">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb breadcrumb-dots mb-0">
                                <li class="breadcrumb-item"><a href="index.php">صفحه اصلی</a></li>
                                <li class="breadcrumb-item"><a href="#">همه دوره ها</a></li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
<!--                </div>-->
                ...
                <div class="">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-sm-6 col-md-3 pb-2 pb-md-0">
                            ...
                            <select name="category" onchange="filter_category(this.value)" class="form-select form-select-sm js-choice" aria-label=".form-select-sm example">
                                <option value=""> همه دسته بندی ها</option>

                                <?php
                                $result_categori= $link->query("SELECT * FROM course_category");
                                while ($row_categori = $result_categori->fetch_assoc()) {
                                    echo '<option value="'.$row_categori['category_id'].'">'.$row_categori['category_name'].'</option>';
                                }
                                ?>
                            </select>
                        </div>
                                ...
                                <div class="card shadow h-100">
                                    ...
                                    <div class="card-body d-flex flex-column justify-content-between">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-0">
                                            ...
                                            <a href="#" class="badge bg-info bg-opacity-10 text-info me-2"><i class="fas fa-circle small fw-bold"></i><?php  echo ' '.$row_course['category_name']; ?> </a>
                                            <!-- Price -->
                        ...
                        <nav class="mt-4 d-flex justify-content-center" aria-label="navigation">
                            <ul class="pagination pagination-primary-soft d-inline-block d-md-flex rounded mb-0">
                                ...
                                for ($i = 1; $i <= $pages_number; $i++) {?>
                                    <li class="page-item mb-0 <?php if ($i==1){echo ' active ';} ?>"><a class="page-link" href="index.php?page=all_course&pageination=<?php echo $i ; ?>"><?php echo $i; ?></a></li>
        ...
        <script>
            function filter_price(str) {
                ...
                xhttp.open("POST", "index.php?page=filter_price&q="+str);
                xhttp.send();
            }
        </script>
        <script>
            function filter_level(str) {
                ...
                xhttp.open("POST", "index.php?page=filter_level&q="+str);
                xhttp.send();
            }
        </script>
        <script>
            function filter_sort(str) {
                ...
                xhttp.open("POST", "index.php?page=filter_sort&q="+str);
                xhttp.send();
            }
        </script>
